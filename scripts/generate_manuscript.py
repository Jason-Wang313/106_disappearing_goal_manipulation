...
PRIMARY = "risk_calibrated_goal_belief_revision_v5"
V4 = "proposed_disappearing_goal_belief_revision_v4"
ORACLE = "oracle_goal_state_supervisor"
...
def reference_score(row: dict[str, str]) -> int:
    ...
    text = " ".join([title, abstract, terms, query])
    bad = [
        "medical",
        "patient",
        "clinical",
    ...
    score = sum(1 for term in anchors if term in text)
    if score == 0:
        return -999
    positives = [
        "uncertainty",
    ...
    return score + sum(1 for term in positives if term in text)
...
def generated_bib(limit: int = 95) -> tuple[list[str], str]:
    ...
    bib = "\n\n".join(entry for _, entry in canonical_bib()) + "\n\n" + "\n\n".join(entries)
    return keys, bib


def write_bib() -> list[str]:
    generated_keys, bib = generated_bib()
    (PAPER / "references.bib").write_text(bib, encoding="utf-8")
    return [key for key, _ in canonical_bib()] + generated_keys


def cite_some(keys: list[str], start: int, count: int = 4) -> str:
...
def metric(payload: dict, name: str) -> str:
    return fnum(payload["primary_metrics"][name], 5)


def strongest_metric(payload: dict, name: str) -> str:
    return fnum(payload["strongest_non_oracle_metrics"][name], 5)


def oracle_metric(payload: dict, name: str) -> str:
    return fnum(payload["oracle_metrics"][name], 5)


def make_tex(keys: list[str]) -> str:
    payload = read_json(RESULTS / "summary.json")
    counts = payload["row_counts"]
    gates = payload["gates"]
    primary = payload["primary_metrics"]
    strong = payload["strongest_non_oracle_metrics"]
    strongest = payload["strongest_non_oracle_baseline"]
    pagesafe_title = "Disappearing Goals in Manipulation via Risk-Calibrated Belief Revision"
    terminal_decision = latex_escape(payload["terminal_decision"])

    intro_cites = cite_some(keys, 0, 5)
    related_cites_a = cite_some(keys, 5, 7)
    related_cites_b = cite_some(keys, 12, 7)
    related_cites_c = cite_some(keys, 19, 7)
    appendix_cites = cite_some(keys, 26, 9)
    protocol_cites = cite_some(keys, 35, 8)
    validation_cites = cite_some(keys, 43, 8)
    stress_cites = cite_some(keys, 51, 8)
...
\begin{{abstract}}
Manipulation systems often treat a last-seen object goal as valid until a low-level controller fails. That shortcut is brittle when a goal disappears because it is occluded, moved, physically removed, temporarily obstructed by a human, invalidated by a changed instruction, delayed in reappearance, or replaceable by a substitute. This paper rebuilds Paper 106 as a hostile-review audit of explicit goal-validity belief revision. The v5 protocol uses {payload['design']['tasks']} task families, {payload['design']['regimes']} disappearing-goal regimes, {payload['design']['splits']} splits, {payload['design']['methods']} methods, {payload['design']['seeds']} seeds, {payload['design']['episodes_per_cell']} episodes per factorial cell, {counts['main_rollout_rows']:,} main rollout rows, ablations, stress sweeps, fixed-risk deployment budgets, and negative cases. The proposed {latex_escape(PRIMARY)} reaches hard-aggregate success {metric(payload, 'success')} versus {strongest_metric(payload, 'success')} for {latex_escape(strongest)}, improves goal-validity F1 from {strongest_metric(payload, 'goal_validity_f1')} to {metric(payload, 'goal_validity_f1')}, reduces stale-goal pursuit from {strongest_metric(payload, 'stale_goal_pursuit')} to {metric(payload, 'stale_goal_pursuit')}, reduces unsafe reach from {strongest_metric(payload, 'unsafe_reach')} to {metric(payload, 'unsafe_reach')}, and improves utility from {strongest_metric(payload, 'utility')} to {metric(payload, 'utility')}. The method still trails the oracle ({oracle_metric(payload, 'success')} success) and the scope gate fails: there is no real robot study, accepted high-fidelity benchmark, external disappearing-goal dataset, trained checkpoint, calibrated deployment log, or rollout video. The honest terminal decision is therefore \textbf{{{terminal_decision}}}, not ready for ICLR main.
\end{{abstract}}

\section{{Why Disappearing Goals Are Not Just Occlusion}}
Goal disappearance is usually collapsed into partial observability, but manipulation creates several physically distinct causes. A mug may be behind a box, moved by a person, removed from the scene, made invalid by an instruction update, replaced by an acceptable substitute, or hidden by a hand that will clear soon. Belief-space planning, active perception, task-and-motion planning, goal-conditioned learning, and large-scale robot policies all provide pieces of the solution {intro_cites}. The missing distinction is not whether the robot is uncertain; it is whether the last goal remains physically and semantically valid.
...
The paper also treats intervention cost as a first-class quantity. The proposed method is allowed to spend more intervention budget only if it buys success, goal validity, and safety. In this run, intervention cost increases from {strongest_metric(payload, 'intervention_cost')} for the strongest non-oracle reference to {metric(payload, 'intervention_cost')} for v5, so the evidence should be read as a safety/validity tradeoff rather than free performance.
...
The experiment is generated by \texttt{{src/run\_experiment.py}}. The design is intentionally factorial: {payload['design']['tasks']} tasks, {payload['design']['regimes']} regimes, {payload['design']['splits']} splits, {payload['design']['methods']} methods, {payload['design']['seeds']} seeds, and {payload['design']['episodes_per_cell']} episodes per cell. The main run stores {counts['dataset_summary_rows']:,} dataset summary rows, {counts['main_rollout_rows']:,} raw main rollout rows, {counts['main_group_rows']:,} group rows, {counts['main_metric_rows']:,} method/split metric rows, and {counts['hard_pairwise_rows']} paired comparisons.
...
{table_block('hard_aggregate_table.tex')}

{figure_block('disappearing_v5_hard_success.png', 'Hard-aggregate success across all methods. The v5 method improves over the v4 belief-revision baseline but remains below the oracle.', 'fig:hard-success')}

The strongest non-oracle baseline is \texttt{{{latex_escape(strongest)}}}. V5 reaches hard-aggregate success {metric(payload, 'success')} versus {strongest_metric(payload, 'success')}, a margin of {fnum(gates['success_margin_vs_strongest'], 5)}. It improves goal-validity F1 by {fnum(gates['goal_validity_f1_delta_vs_strongest'], 5)}, reduces stale-goal pursuit by {fnum(-gates['stale_goal_pursuit_delta_vs_strongest'], 5)}, reduces unsafe reach by {fnum(-gates['unsafe_reach_delta_vs_strongest'], 5)}, reduces false abandonment by {fnum(-gates['false_abandonment_delta_vs_strongest'], 5)}, and improves utility by {fnum(gates['utility_margin_vs_best_non_oracle'], 5)} over the best non-oracle utility baseline. The oracle still reaches {oracle_metric(payload, 'success')} success, leaving a visible headroom gap.

\section{{Diagnostics and Calibration}}
{figure_block('disappearing_v5_diagnostics.png', 'Goal-validity F1 and stale-goal pursuit. V5 is not merely more conservative: it increases validity F1 while lowering stale pursuit.', 'fig:diagnostics')}

Goal-validity F1 is the key diagnostic because success alone could be purchased by unsafe persistence or frequent halting. The v5 method reaches {metric(payload, 'goal_validity_f1')} F1 while reducing stale-goal pursuit to {metric(payload, 'stale_goal_pursuit')}. The false-abandonment rate is {metric(payload, 'false_abandonment')}, lower than the strongest non-oracle baseline at {strongest_metric(payload, 'false_abandonment')}. ECE is {metric(payload, 'ece')}, passing the frozen calibration gate.

{figure_block('disappearing_v5_safety_regret.png', 'Unsafe reach and regret to oracle. V5 reduces unsafe reach relative to all non-oracle baselines but does not close the oracle gap.', 'fig:safety-regret')}

\section{{Paired Seed Tests}}
{table_block('pairwise_decision_table.tex')}

Paired seed tests compare v5 to every method over the hard aggregate. V5 beats the strongest non-oracle baseline by {fnum(gates['success_margin_vs_strongest'], 5)} success and {fnum(gates['utility_margin_vs_best_non_oracle'], 5)} utility. The comparison against the oracle is intentionally negative. That is a healthy sign: the benchmark is not saturated, and the proposed method is not being presented as omniscient.

\section{{Ablations}}
{table_block('ablation_table.tex')}

{figure_block('disappearing_v5_ablation.png', 'Ablation success on hard splits. The full v5 system is strongest on utility even when the success-only margin to the intervention-utility ablation is small.', 'fig:ablations')}

The full method beats every removed-component variant on success or utility. The closest success ablation is \texttt{{{latex_escape(gates['best_removed_component_success'])}}}, with a success margin of {fnum(gates['ablation_success_margin_vs_best_removed_component'], 5)}. The closest utility ablation is \texttt{{{latex_escape(gates['best_removed_component_utility'])}}}, with a utility margin of {fnum(gates['ablation_utility_margin_vs_best_removed_component'], 5)}. This is a deliberately uncomfortable result: the success margin alone is small, so the claim should be framed as a utility-calibrated belief-revision mechanism rather than a dramatic raw-success breakthrough.

\section{{Stress Sweep}}
{table_block('stress_table.tex')}

{figure_block('disappearing_v5_stress_sweep.png', 'Stress sweep over occlusion, removal, delay, retarget ambiguity, human obstruction, and task-change pressure.', 'fig:stress')}

At maximum stress level, v5 retains a success margin of {fnum(gates['stress_margin_at_max_level'], 5)} over the strongest non-oracle stress reference, \texttt{{{latex_escape(gates['max_stress_non_oracle_reference'])}}}. The stress sweep is important because disappearing-goal mechanisms often fail only when object absence, task change, and delayed observations combine. A pure occlusion benchmark would overstate the value of active perception alone.

\section{{Fixed-Risk Deployment Budgets}}
{table_block('fixed_risk_table.tex')}

{figure_block('disappearing_v5_fixed_risk.png', 'Fixed-risk utility across intervention budgets. V5 keeps nontrivial coverage and better utility at the strict budget used by the frozen gate.', 'fig:fixed-risk')}

At intervention budget 0.18, v5 has coverage {fnum(gates['fixed_risk_coverage_at_budget_0_18'], 5)} and utility margin {fnum(gates['fixed_risk_utility_margin_at_budget_0_18'], 5)} over \texttt{{{latex_escape(gates['fixed_risk_reference_at_budget_0_18'])}}}. This matters because a robot can appear safe by refusing to act. The fixed-risk gate requires both nontrivial coverage and utility improvement.

\section{{Negative and Boundary Cases}}
{table_block('negative_cases_table.tex')}
...
Belief-space task-and-motion planning and POMDP planning provide the conceptual background for updating plans under hidden state {related_cites_a}. Active perception and exploration provide mechanisms for reducing occlusion uncertainty {related_cites_b}. Goal-conditioned learning, HER, language-conditioned manipulation, and broad robot policy datasets show that goals can be represented and reused in powerful ways {related_cites_c}. Recovery RL, safety filtering, conformal prediction, robust MPC, and failure-aware manipulation provide relevant safety baselines {appendix_cites}.
...
The local gates pass: success, goal-validity, stale-goal, unsafe-reach, false-abandonment, intervention-cost disclosure, calibration, utility, pairwise, ablation, stress, and fixed-risk gates are all true. The scope gate is false. The run has no real robot, no accepted high-fidelity manipulation benchmark, no external disappearing-goal benchmark, no learned checkpoint, no calibrated deployment log, and no rollout video. Therefore the terminal state is \textbf{{{terminal_decision}}}; ICLR main ready is \textbf{{no}}.

\clearpage
\appendix

\section{{Appendix A: Full Artifact Inventory}}
The evidence package includes {counts['main_rollout_rows']:,} main rollouts, {counts['main_group_rows']:,} main group rows, {counts['main_seed_metric_rows']:,} main seed rows, {counts['hard_seed_rows']:,} hard-aggregate seed rows, {counts['ablation_rollout_rows']:,} ablation rollouts, {counts['stress_rollout_rows']:,} stress rollouts, {counts['fixed_risk_rows']:,} fixed-risk rows, and {counts['failure_case_rows']} negative cases. The canonical numbered PDF is written only to \texttt{{C:/Users/wangz/Downloads/106.pdf}}.
...
Gate quantity & Value \\
\midrule
Success margin vs strongest & {fnum(gates['success_margin_vs_strongest'], 5)} \\
Goal-validity F1 delta & {fnum(gates['goal_validity_f1_delta_vs_strongest'], 5)} \\
Stale-goal delta & {fnum(gates['stale_goal_pursuit_delta_vs_strongest'], 5)} \\
Unsafe-reach delta & {fnum(gates['unsafe_reach_delta_vs_strongest'], 5)} \\
False-abandonment delta & {fnum(gates['false_abandonment_delta_vs_strongest'], 5)} \\
Intervention-cost delta & {fnum(gates['intervention_cost_delta_vs_strongest'], 5)} \\
Utility margin & {fnum(gates['utility_margin_vs_best_non_oracle'], 5)} \\
Stress margin & {fnum(gates['stress_margin_at_max_level'], 5)} \\
Fixed-risk coverage & {fnum(gates['fixed_risk_coverage_at_budget_0_18'], 5)} \\
Fixed-risk utility margin & {fnum(gates['fixed_risk_utility_margin_at_budget_0_18'], 5)} \\
\bottomrule
\end{{tabular}}
\end{{table}}
...
\paragraph{{ECE.}}
ECE measures calibration of the validity estimate. The v5 run obtains {metric(payload, 'ece')}, below the frozen threshold.

\paragraph{{Regret to oracle.}}
Regret to oracle makes the remaining gap explicit. V5 regret is {metric(payload, 'regret')}; the method is not close to omniscience.
...
The strict gate uses budget 0.18 because it is low enough to punish indiscriminate probing and high enough to allow meaningful recovery. V5 reaches coverage {fnum(gates['fixed_risk_coverage_at_budget_0_18'], 5)} at this budget. A zero-coverage policy would be safe but useless; a full-coverage policy would ignore the budget. The reported coverage shows that v5 is neither.

The strongest fixed-risk reference is \texttt{{{latex_escape(gates['fixed_risk_reference_at_budget_0_18'])}}}. V5 beats it by {fnum(gates['fixed_risk_utility_margin_at_budget_0_18'], 5)} utility at the strict budget. This result should be interpreted locally: it is evidence that the mechanism is coherent under the generator, not evidence that a real robot controller is deployment-certified.
...
The literature slice contains partially observable planning, task-and-motion planning, goal retargeting, active perception, robot recovery, language-conditioned manipulation, and robot foundation model work {protocol_cites}. The paper therefore cannot claim a broad discovery. Its claim is narrower: the disappearing-goal taxonomy and the fixed-risk belief-revision audit expose a gap between generic uncertainty handling and action-specific validity reasoning.

The most threatening prior work is not any single paper. It is the combination of belief-space planning, active perception, retargeting, and recovery under uncertainty {validation_cites}. A reviewer could reasonably ask whether those pieces, implemented carefully, already solve the problem. The v5 baseline suite is designed around that objection.

The local pool also contains work on benchmarks and high-fidelity robot manipulation {stress_cites}. That literature is precisely why the scope gate fails. Without external validation on such benchmarks or hardware, this paper remains a strong revise package rather than a submission-ready result.

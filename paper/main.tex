        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Disappearing Goal Manipulation: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study partially observable manipulation. The submission-hardened claim is intentionally narrow: Handle goals that become physically or perceptually unavailable during execution. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Disappearing goal manipulation should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.411 $\pm$ 0.022 & 0.441 $\pm$ 0.016 & 0.322 $\pm$ 0.010 \\
Data-augmented WAM & 0.499 $\pm$ 0.024 & 0.501 $\pm$ 0.010 & 0.405 $\pm$ 0.012 \\
Scalar uncertainty & 0.573 $\pm$ 0.013 & 0.576 $\pm$ 0.020 & 0.507 $\pm$ 0.030 \\
Ensemble disagreement & 0.582 $\pm$ 0.024 & 0.609 $\pm$ 0.011 & 0.529 $\pm$ 0.012 \\
Conformal risk filter & 0.624 $\pm$ 0.021 & 0.624 $\pm$ 0.012 & 0.557 $\pm$ 0.012 \\
Proposed branch mechanism & 0.691 $\pm$ 0.010 & 0.704 $\pm$ 0.013 & 0.654 $\pm$ 0.011 \\
Oracle branch upper bound & 0.764 $\pm$ 0.019 & 0.773 $\pm$ 0.009 & 0.744 $\pm$ 0.013 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.669 $\pm$ 0.015 & all components \\
Minus branch atlas & 0.558 $\pm$ 0.022 & removes explicit unrealized futures \\
Minus tail risk & 0.578 $\pm$ 0.014 & uses mean predicted value only \\
Minus diagnostic probe & 0.594 $\pm$ 0.010 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.605 $\pm$ 0.021 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/106.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Disappearing Goal Manipulation is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
